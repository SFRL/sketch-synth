...
class ADSR {
   ...
   */
  release() {
    if (this.state_ === ADSRStates.RELEASE || this.state_ === ADSRStates.SILENT)
      return;
    ...
    for (let output of outputs) {
      for (let channel of output) {
        for (let n = 0; n < channel.length; n++) {
          ...
          // hardcoded for now.
          const op3_out =
            6.283 * env3_amp * params.gain_3[0] * Math.sin(this.op3_phase_);
          const op2_out =
            6.283 * env2_amp * params.gain_2[0] * Math.sin(this.op2_phase_);
          const op1_out =
            env1_amp *
            params.gain_1[0] *
            Math.sin(this.op1_phase_ + op2_out + op3_out);

          // Update phase of each operator
          const op1_multiplier = params.coarse_1[0] + 0.001 * params.fine_1[0];
          this.op1_phase_ +=
            (op1_multiplier * 2.0 * Math.PI * this.note_freq) / sampleRate;
          while (this.op1_phase_ >= 2.0 * Math.PI) {
            this.op1_phase_ -= 2.0 * Math.PI;
          }

          const op2_multiplier = params.coarse_2[0] + 0.001 * params.fine_2[0];
          this.op2_phase_ +=
            (op2_multiplier * 2.0 * Math.PI * this.note_freq) / sampleRate;
          while (this.op2_phase_ >= 2.0 * Math.PI) {
            this.op2_phase_ -= 2.0 * Math.PI;
          }

          const op3_multiplier = params.coarse_3[0] + 0.001 * params.fine_3[0];
          this.op3_phase_ +=
            (op3_multiplier * 2.0 * Math.PI * this.note_freq) / sampleRate;
