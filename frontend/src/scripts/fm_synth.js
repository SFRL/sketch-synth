...
class FMSynth {
  ...
  async initialize() {
    ...
    this.node_ = new AudioWorkletNode(this.context_, "fm_synth_processor");
  ...
  setParam(paramName, value, rampTime) {
    ...
    else this.true_params[paramName] = value;
  }
  ...
  getAllParams() {
    if (!this.using_true_params) return this.true_params;
