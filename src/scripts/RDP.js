...
var simplifyPath = function (inX, inY, tol) {
  var diff = function (ux, uy, vx, vy) {
    return [ux - vx, uy - vy];
  };
  var dot = function (ux, uy, vx, vy) {
    return ux * vx + uy * vy;
  };
  var norm2 = function (vx, vy) {
    return vx * vx + vy * vy;
  };
  var d2 = function (ux, uy, vx, vy) {
    var d = diff(ux, uy, vx, vy);
    return norm2(d[0], d[1]);
  };
  ...
  var simplifyDP = function (tol, x, y, j, k, mk) {
    ...
    for (var i = j + 1; i < k; i++) {
      ...
      if (cw <= 0) {
        ...
        b = cw / cu;
        var Pbx = segStart[0] + b * u[0];
        var Pby = segStart[1] + b * u[1];
